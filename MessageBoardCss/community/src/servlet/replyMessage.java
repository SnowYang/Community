...
public class replyMessage extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		int messageid=Integer.parseInt(request.getParameter("replyid"));
		
		ReplyMessage replymessage=new ReplyMessage();
		
		replymessage.setMessageid(messageid);
		replymessage.setReplyusername(replyusername);
		replymessage.setReplydate(new Date(System.currentTimeMillis()));
		replymessage.setReplycontent(replycontent);
		
		DB db=new DB();
		
		if(db.addReplyMessage(replymessage))
		{
		response.sendRedirect("/MessageBoardCss/ReplySuccess.jsp");
		}
		else
		{
			out.println("≤Â»Î ß∞‹£°");
